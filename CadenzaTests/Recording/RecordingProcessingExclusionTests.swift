...
struct RecordingProcessingExclusionTests {
    ...
    @Test func submittedJobRefusesManualAndAutomaticStartBeforeEveryBoundary() async throws {
        let isolated = makeDefaults()
        defer { isolated.value.removePersistentDomain(forName: isolated.name) }
        let gate = RecordingProcessingGate()
        let spy = RecordingProcessingBoundarySpy()
        ...
        #expect(persistedIDs == [fixture.id])
    ...
    @Test func manualSummaryLeaseBlocksManualAndDetectedAutoCapture() async throws {
        try await verifyManualProcessingLease(.manualSummary)
    }

    @Test func manualRetryLeaseBlocksManualAndDetectedAutoCapture() async throws {
    ...
    private func verifyManualProcessingLease(
        ...
        #expect(engine._test_pendingMeetingAutoStart?.bundleID == "com.example.meeting")
        #expect(spy.providerChecks == 0)
        #expect(spy.permissionRequests == 0)
        #expect(spy.captureRequests.isEmpty)
        #expect(await store.fetchRecordingDTOs(
        ...
        engine.handleMeetingTerminated(bundleID: "com.example.meeting")
    ...
    @Test func pendingMeetingSurvivesPriorTriggerTerminationAndStartsOnceAtAllIdle() async throws {
        let isolated = makeDefaults()
        defer { isolated.value.removePersistentDomain(forName: isolated.name) }
        SystemAudioCapturePreparation.markPrepared(defaults: isolated.value)
        let gate = RecordingProcessingGate()
    ...
    @Test func endedPendingMeetingIsNotRetriedWhenProcessingBecomesIdle() async throws {
        let isolated = makeDefaults()
        defer { isolated.value.removePersistentDomain(forName: isolated.name) }
        let gate = RecordingProcessingGate()
        let spy = RecordingProcessingBoundarySpy()

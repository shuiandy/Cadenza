...
**让会议变成真正可用的知识，而不是变成别人的数据。**

Cadenza 是一款为 Mac 打造的隐私优先会议工作台。它可以捕获系统音频、在本地完成转录、
记住说话人，并把对话整理成可搜索的决策与行动项。在受支持的 Mac 上，从录音、转录、
总结、聊天到导出的完整流程都可以在本地运行，不需要 Cadenza 账号，也不依赖托管后端。

**需要隐私时全程本地，需要选择时自由切换服务商，并为未来自托管 Web 端保留路径。**
...
![本地优先](https://img.shields.io/badge/local--first-privacy-34c759?style=flat-square)
...
大多数会议助手从托管服务开始，Cadenza 则从你的 Mac 开始。录音、整理、转录、搜索和导出
都不要求注册 Cadenza 账号，也不依赖 Cadenza 托管的后端。当你启用会议检测后，它会安静地
在后台运行，判断会议是否真的正在进行，并生成原本需要你手动整理的内容，包括会议概览、
已经做出的决策，以及每个人接下来要完成的事项。
...
会议仍在进行时，实时转录会持续显示在录音浮层中。说话人分离通过 SpeakerKit 在设备端运行；
一旦你为说话人命名，Cadenza 可以在后续会议中记住他们。

<img src="docs/screenshots/03-transcript.jpg" alt="转录视图，包含说话人分布和按说话人归类的时间戳片段" width="100%">
